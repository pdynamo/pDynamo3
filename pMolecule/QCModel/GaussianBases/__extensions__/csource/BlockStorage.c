...
! . In some aspects block storage behaves like a sparse array and it would be
! . ad
!=================================================================================================================================*/

/*----------------------------------------------------------------------------------------------------------------------------------
! . Block storage behaves in many ways like a generalized sparse array.
! . It would be advantageous to extend this behavior so that it could be more widely used in the code!
!---------------------------------------------------------------------------------------------------------------------------------*/
...
/*----------------------------------------------------------------------------------------------------------------------------------
! . Iterate over the storage blocks.
!---------------------------------------------------------------------------------------------------------------------------------*/
...
!---------------------------------------------------------------------------------------------------------------------------------*/
void BlockStorage_Print ( BlockStorage *self )
{
    if ( self != NULL )
    {
        auto Block  *block ;
        auto Integer c, i, i16, i32, iR, n = 0, n16 = self->nIndices16, n32 = self->nIndices32, nR = self->nReal ;
        List_Iterate_Initialize ( self->blocks ) ;
        while ( ( block = BlockStorage_Iterate ( self ) ) != NULL )
        {
            for ( c = i16 = i32 = iR = 0 ; c < block->count ; c++, i16 += n16, i32 += n32, iR += nR, n++ )
            {
                printf ( "%6d |", n ) ;
                if ( n16 > 0 )
                {
                    for ( i = 0 ; i < n16 ; i++ ) printf ( " %6d", block->indices16[i16+i] ) ;
                    printf ( " |" ) ;
                }
                if ( n32 > 0 )
                {
                    for ( i = 0 ; i < n32 ; i++ ) printf ( " %6d", block->indices32[i32+i] ) ;
                    printf ( " |" ) ;
                }
                if ( nR > 0 )
                {
                    for ( i = 0 ; i < nR ; i++ ) printf ( " %20.10f", block->data[iR+i] ) ;
                }
                printf ( "\n" ) ;
            }
        }
