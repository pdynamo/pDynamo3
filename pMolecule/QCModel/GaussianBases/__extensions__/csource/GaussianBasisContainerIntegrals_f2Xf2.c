...
/*
  ...
  Note that O4 has more symmetry than A4/C4:

  - for A4/C4: <ab||cd> = <ab||dc> = <ba||cd> = <ba||dc>, etc. (8 cases)
  - for O4   : <ab||cd> = <ac||bd> = <ad||bc>, etc. as well (24 cases)

    This means O4 could be much cheaper to calculate although would need to have additional FockConstruction methods.
    Can still use old methods. Note A4/C4 will also have the same problem when all functions are on the same center.

  Gaussian product theorem for four Gaussians (easy to generalize to more):

  - O4 overlap = exp [ - (a1 a2 R12^2 + a1 a3 R13^2 + a1 a4 R14^2 + a2 a3 R23^2 + a2 a4 R24^2 + a3 a4 R34^2)/A ] *
                 exp [ - A (r-( a1 R1 + a2 R2 + a3 R3 + a4 R4 )/A)^2 ]
    with A = a1 + a2 + a3 + a4
...
/*----------------------------------------------------------------------------------------------------------------------------------
! . The Coulomb two-electron integral derivatives.
!---------------------------------------------------------------------------------------------------------------------------------*/
...
/*----------------------------------------------------------------------------------------------------------------------------------
! . Calculate the two-electron integrals with choice of operator.
! . Duplicated as this is currently just a test method.
!---------------------------------------------------------------------------------------------------------------------------------*/
# define _TEIs_BlockSize 1024
# define _TEIs_UnderFlow 1.0e-12
void GaussianBasisContainerIntegrals_f2Xf2i ( const GaussianBasisContainer *self         ,
                                              const Coordinates3           *coordinates3 ,
                                                    ...
                                                    Status                 *status       )
{
         ...
         Status_IsOK ( status ) )
    {
        ...
        auto Integer        c, i, i0, *iWork, j, j0, k, k0, l, l0, n, s4 ;
        auto GaussianBasis *iBasis, *jBasis, *kBasis, *lBasis ;
        auto Real           d, *rI, rIJ[3], rIJ2 = 0.0e+00, *rJ, *rK, rKL[3], rKL2 = 0.0e+00, *rL, *rWork ;
        /* . Initialization. */
        BlockStorage_Empty ( teis ) ;
        teis->blockSize      = _TEIs_BlockSize ;
        teis->checkUnderFlow = True ;
        ...
        s4    = n*n*n*n ;
        iWork = Integer_Allocate ( 3*s4, status ) ;
        if ( operator == GaussianBasisOperator_Overlap ) rWork = Real_Allocate ( 2*s4, status ) ;
        else                                             rWork = Real_Allocate ( 3*s4, status ) ;
        if ( ! Status_IsOK ( status ) ) goto FinishUp ;
        /* . Quadruple loop over centers. */
        for ( i = 0 ; i < self->capacity ; i++ )
        {
...
/* . Need flag for j < l. */
                        ...
                        {
                            GaussianBasisIntegrals_f2Af2i ( iBasis, rI, jBasis, rJ, rIJ, rIJ2, kBasis, rK, lBasis, rL, rKL, rKL2, ( j < l ), s4, iWork, rWork, block ) ;
                        }
                        ...
                        {
                            GaussianBasisIntegrals_f2Of2i ( iBasis, rI, jBasis, rJ,            kBasis, rK, lBasis, rL,            ( j < l ), s4, iWork, rWork, block ) ;
                        }
                        ProcessTEIs ( i0, j0, k0, l0, block, teis, status ) ;
                        if ( ! Status_IsOK ( status ) ) goto FinishUp ;
                    }
