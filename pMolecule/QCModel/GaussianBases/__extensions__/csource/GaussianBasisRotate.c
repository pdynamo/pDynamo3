                                              ...
                                              Status        *status )
{
         ...
         Status_IsOK ( status ) )
    {
        ...
        isOK = ( ( View2D_Columns ( Tc ) >= n            ) &&
                 ( View2D_Rows    ( Tc ) >= n            ) &&
                 ( View2D_Columns ( T  ) == self->nBasis ) &&
                 ( View2D_Rows    ( T  ) == self->nBasis ) ) ;
        if ( isOK )
        {
            auto RealArray2D *Tl = T, *Tt = NULL ;
            /* . This is normally way too big as needed only per shell. */
            if ( self->isSpherical ) Tt = RealArray2D_AllocateWithExtents ( self->nBasis, self->nCBF, status ) ;
            if ( Status_IsOK ( status ) )
            {
                auto Integer     iShell, lMin, lStart, nC, start ;
                auto RealArray2D cView, lView, tView ;
                RealArray2D_Set ( Tl, 0.0e+00 ) ;
                for ( iShell = 0 ; iShell < self->nShells ; iShell++ )
                {
                    lMin   = self->shells[iShell].lLow ;
/*
printf ( "\nShell c2s and s2c - %d %d %d %d:\n", self->atomicNumber, iShell, lMin, self->shells[iShell].lHigh ) ; fflush ( stdout ) ;
RealArray2D_Print ( self->shells[iShell].c2s ) ; fflush ( stdout ) ;
RealArray2D_Print ( self->shells[iShell].s2c ) ; fflush ( stdout ) ;
*/
                    lStart = ( lMin * ( lMin + 1 ) * ( lMin + 2 ) ) / 6 ;
                    start  = self->shells[iShell].nStart       ;
                    n      = self->shells[iShell].nBasis ;
                    nC     = self->shells[iShell].nCBF   ;
                    RealArray2D_View ( Tc, lStart, lStart, nC, nC, 1, 1, False, &cView, status ) ;
                    RealArray2D_View ( Tl,  start,  start, n , n , 1, 1, False, &lView, status ) ;
                    if ( self->isSpherical )
                    {
                        RealArray2D_View ( Tt, 0, 0, n, nC, 1, 1, False, &tView, status ) ;
                        RealArray2D_MatrixMultiply ( True , False, 1.0e+00, self->shells[iShell].c2s, &cView, 0.0e+00, &tView, status ) ;
                        RealArray2D_MatrixMultiply ( False, False, 1.0e+00, &tView, self->shells[iShell].s2c, 0.0e+00, &lView, status ) ;
                    }
                    else
                    {
            ...
            if ( self->isSpherical ) RealArray2D_Deallocate ( &Tt ) ;
/*
printf ( "\nTc and T:\n" ) ; fflush ( stdout ) ;
RealArray2D_Print ( Tc ) ; fflush ( stdout ) ;
RealArray2D_Print ( T  ) ; fflush ( stdout ) ;
*/
        }
        else Status_Set ( status, Status_NonConformableArrays ) ;
    }
