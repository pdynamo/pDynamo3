...
# include "GaussianBasisContainerIntegrals_b4e2n0.h"
...
void GaussianBasisContainerIntegrals_TEIs ( const GaussianBasisContainer *self         ,
                                                  ...
                                                  Status                 *status       )
{
    ...
    {
        auto Block         *block ;
        auto Integer        c, i, i0, j, j0, k, k0, l, l0, n ;
        auto GaussianBasis *iBasis, *jBasis, *kBasis, *lBasis ;
        auto Real           d, *rI, rIJ[3], rIJ2, *rJ, *rK, rKL[3], rKL2, *rL ;
        /* . Initialization. */
        ...
        for ( i = 0 ; i < self->capacity ; i++ )
        {
            ...
            for ( j = 0 ; j <= i ; j++ )
            {
                ...
                for ( k = 0 ; k <= i ; k++ )
                {
                    ...
                    k0     = Array1D_Item ( basisIndices, k ) ;
                    rK     = Coordinates3_RowPointer ( coordinates3, k ) ;
                    for ( l = 0 ; l <= k ; l++ )
                    {
                        lBasis = self->entries[l] ;
                        l0     = Array1D_Item ( basisIndices, l ) ;
                        rL     = Coordinates3_RowPointer ( coordinates3, l ) ;
                        for ( c = 0, rKL2 = 0.0e+00 ; c < 3 ; c++ ) { d = rK[c] - rL[c] ; rKL[c] = d ; rKL2 += d * d ; }
# ifdef _PrintIntegrals
printf ( "Atoms %6d %6d %6d %6d | Basis Indices %6d %6d %6d %6d\n", i, j, k, l, i0, j0, k0, l0 ) ;
fflush ( stdout ) ;
# endif
/* . Need flag for j < l. */
                        GaussianBasisIntegrals_TEIs ( iBasis, rI, jBasis, rJ, rIJ, rIJ2, kBasis, rK, lBasis, rL, rKL, rKL2, ( j < l ), block ) ;
...
!---------------------------------------------------------------------------------------------------------------------------------*/
void GaussianBasisContainerIntegrals_TEIsD ( const GaussianBasisContainer *self            ,
                                             const IntegerArray1D         *basisIndices    ,
                                             const Coordinates3           *coordinates3    ,
                                             const SymmetricMatrix        *dTotal          ,
                                             const SymmetricMatrix        *dSpin           ,
            ...
            for ( j = 0 ; j <= i ; j++ )
            {
                ...
                for ( k = 0 ; k <= i ; k++ )
                {
                    ...
                    for ( l = 0 ; l <= k ; l++ )
                    {
                        ...
                        for ( c = 0, rKL2 = 0.0e+00 ; c < 3 ; c++ ) { d = rK[c] - rL[c] ; rKL[c] = d ; rKL2 += d * d ; }
/* . Need flag for j < l. */
                        GaussianBasisIntegrals_TEIsD ( iBasis, rI, jBasis, rJ, rIJ, rIJ2, kBasis, rK, lBasis, rL, rKL, rKL2, ( j < l ), block ) ;
