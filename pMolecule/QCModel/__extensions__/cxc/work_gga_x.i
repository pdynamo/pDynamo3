...
/* . MJF. */
/*      gdm    = max(SQRT(max(sigma[js],0.0e+00))/sfact, p->info->min_grad);*/
...
/*if ( ip == 1562 && is == 1 ) printf ( "\nVRHO BEFORE> %5d %40.20f %40.20f\n", ip, rho[is], vrho[is] ) ;*/
	vrho[is] += x_factor_c*(rhoLDA/ds)*(alpha*f - beta*dfdx*x)
	  + x_factor_c*rhoLDA*lvrho;
/*
if ( ip == 1562 && is == 1 )
{
printf ( "\nVRHO AFTER> %5d %40.20f %40.20f %40.20f\nVRHO AFTER> %5d %40.20f %40.20f %40.20f\nVRHO AFTER> %5d %40.20f %40.20f %40.20f\nVRHO AFTER> %5d %40.20f %40.20f\n",
                             ip, rho[is], vrho[is], x_factor_c, ip, rhoLDA, ds, alpha, ip, f, beta, dfdx, ip, x, lvrho ) ;
printf ( "\nVRHO AFTER> %5d %40.20f %40.20f %40.20f\nVRHO AFTER> %5d %40.20f %40.20f\n",
                             ip, alpha, beta, gdm, ip, sigma[js], sfact ) ;
}
*/
